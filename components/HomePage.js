    ...
    <main>
      ...
      <section id="home" className="hero-section">
        <div className="container hero-container">
          ...
          <div className="hero-features">{t.features.map(([title,text],i)=><article className="feature-card" key={title}><b>{["✓","ϟ","◎"][i]}</b><div><h3>{title}</h3><p>{text}</p></div></article>)}</div>
        </div>
      </section>

      <section id="about" className="about-section"><div className="container"><SectionTitle eyebrow={t.aboutEyebrow} title={t.aboutTitle}/><div className="about-grid"><div className="about-text"><h3>{t.aboutHead}</h3><p>{t.aboutText}</p><ul className="about-features">{t.points.map(x=><li key={x}>✓ {x}</li>)}</ul></div><div className="about-stats"><div><strong>15+</strong><span>{locale==="tr"?"Desteklenen Dil":"Languages"}</span></div><div><strong>10K+</strong><span>{locale==="tr"?"Çevrilen Belge":"Documents"}</span></div><div><strong>5.0 / 5</strong><span>Google</span></div><div><strong>Online</strong><span>{locale==="tr"?"Türkiye Geneli":"Across Türkiye"}</span></div></div></div></div></section>
